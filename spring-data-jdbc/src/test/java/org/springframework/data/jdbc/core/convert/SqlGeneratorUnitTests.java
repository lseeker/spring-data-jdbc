...
public class SqlGeneratorUnitTests {
	...
	public void cascadingDeleteByPathSecondLevel() {
		...
		assertThat(sql).isEqualTo(
				"DELETE FROM second_level_referenced_entity WHERE second_level_referenced_entity.referenced_entity IN (SELECT referenced_entity.x_l1id FROM referenced_entity WHERE referenced_entity.dummy_entity = :dummy_entity)");
	}
				...
				.createDeleteByPath(getPath("idNoIdChain.chain4.chain3.chain2.chain1.chain0", IdIdNoIdChain.class))) //
						.isEqualTo( //
								"DELETE FROM no_id_chain0 " //
										...
										+ "FROM id_no_id_chain " //
										+ "WHERE id_no_id_chain.id_id_no_id_chain = :id_id_no_id_chain" //
										+ "))");
	}
	...
	public void joinForSimpleReference() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinConditions().get(0).getJoinColumn().getName())
					.isEqualTo(SqlIdentifier.quoted("DUMMY_ENTITY"));
			softly.assertThat(join.getJoinConditions().get(0).getParentId().getName()).isEqualTo(SqlIdentifier.quoted("id1"));
	...
	public void joinForSecondLevelReference() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinConditions().get(0).getJoinColumn().getTable()).isEqualTo(join.getJoinTable());
			softly.assertThat(join.getJoinConditions().get(0).getJoinColumn().getName())
					.isEqualTo(SqlIdentifier.quoted("REFERENCED_ENTITY"));
			softly.assertThat(join.getJoinConditions().get(0).getParentId().getName())
					.isEqualTo(SqlIdentifier.quoted("X_L1ID"));
			softly.assertThat(join.getJoinConditions().get(0).getParentId().getTable().getName())
					.isEqualTo(SqlIdentifier.quoted("REFERENCED_ENTITY"));
		});
	}
	...
	public void joinForOneToOneWithoutId() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinConditions().get(0).getJoinColumn().getTable()).isEqualTo(joinTable);
			softly.assertThat(join.getJoinConditions().get(0).getJoinColumn().getName())
					.isEqualTo(SqlIdentifier.quoted("PARENT_OF_NO_ID_CHILD"));
			softly.assertThat(join.getJoinConditions().get(0).getParentId().getName())
					.isEqualTo(SqlIdentifier.quoted("X_ID"));
			softly.assertThat(join.getJoinConditions().get(0).getParentId().getTable().getName())
					.isEqualTo(SqlIdentifier.quoted("PARENT_OF_NO_ID_CHILD"));
